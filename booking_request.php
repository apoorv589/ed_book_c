...
class Booking_request extends MY_Controller
{
	...
	function app_list($auth)
	{
		...
		$res = $this->edc_booking_model->get_requests ("Pending", $auth, $dept_id);
		...
		$res = $this->edc_booking_model->get_requests ("Approved", $auth, $dept_id);
		...
		$res = $this->edc_booking_model->get_requests ("Rejected", $auth, $dept_id);	//in case of pce_da5, there won't be any rejected apps
		...
		$res = $this->edc_booking_model->get_new_applications ($auth, $dept_id); //function arguments: auth dept_id
	...
	function pce_da5_app_list()
	{
		...
		$res = $this->edc_booking_model->get_requests ("Pending", 'pce_da5', 'all');
		$total_rows_pending = count($res);
		$data_array_pending = array();
		$sno = 1;
		foreach ($res as $row)
		...
		$res = $this->edc_booking_model->get_requests ("Approved", 'pce_da5', 'all');
		$total_rows_approved = count($res);
		$data_array_approved = array();
		$sno = 1;
		foreach ($res as $row)
		...
		$res = $this->edc_booking_model->get_new_applications ('pce_da5', 'all'); //function arguments: auth dept_id
		$total_new_apps = count($res);
		$data_array_new_apps = array();
		$sno = 1;
		foreach ($res as $row)
	...
	function details ($app_num, $auth)
	{
		...
		foreach($res as $row) //it will output only one element corresponding to app_num
		{
			$data = array(
				...
				'pce_action_timestamp' => $row['pce_action_timestamp'],
				'deny_reason' => $row['deny_reason']
			);
		}

		$dept = $this->user_model->getById($this->edc_booking_model->get_request_user_id($app_num))->dept_id; //returns the department of the applicant
	...
	function official_action($app_num, $auth)
	{
		...
		$b_detail = $this->edc_booking_model->get_booking_details($app_num);
		...
		if($b_detail[0]['hod_status'] === 'Cancel' ||
			...
			$b_detail[0]['pce_status'] === 'Cancelled') {
	...
	function cancellation($app_num, $auth)
	{
		...
		$user_auth = $this->edc_booking_model->get_user_auth($app_num)['auth_id'];
		...
		if($auth == 'emp') //applicant can communicate with hod/hos, dsw, ctk only (in case of emp personal)
		{
			...
			{
				$this->edc_booking_model->cancel_request($app_num);
				$this->notification->notify($pce_da5, 'pce_da5', 'EDC Booking Cancellation', 'Request for EDC Room Booking (Application No. : '.$app_num.') has been Cancelled by Applicant', 'edc_booking/booking_request/details/'.$app_num.'/pce_da5');
				$this->notification->notify($data['user_id'], $user_auth, 'EDC Booking Cancellation', 'Request for EDC Room Booking Cancellation (Application No. : '.$app_num.') has been Approved successfully.', 'edc_booking/booking_request/details/'.$app_num.'/'.$user_auth);
				$this->session->set_flashdata('flashSuccess', 'Booking Cancellation has been approved successfully.');
				redirect('edc_booking/booking/history');
			}
