...
class Guest_details extends MY_Controller {
	...
	function edit($app_num='')
	{
		...
		$data['user_id'] = $this->edc_booking_model->get_request_user_id($app_num);

//-->	//the code below finds out the groups and rooms associated to each group
		$res = $this->edc_booking_model->get_guest_groups($app_num);
		$data['guest_details'] = array();
		$sno = 0;
		foreach($res as $row)
		{
			$data['guest_details'][$sno] = $row;
			$room_res = $this->edc_booking_model->get_guest_rooms($row['app_num'], $row['name'], $row['check_in']); //got the room id
			$i = 0;
...
//-->
		...
		$data['count_guest']=count($this->edc_booking_model->get_guest_details($app_num));

		//to show only available rooms from list of allotted rooms,
		// -> get list of allotted rooms
		// -> get rooms that are not full
		$allotted_rooms = $this->edc_allotment_model->get_allocated_room_details($app_num);
		$data['no_of_rooms'] = count($allotted_rooms);
		$sno = 0;
		$data['rooms_left'] = 0;
		foreach($allotted_rooms as $allotted_room)
		{
			$type = $this->edc_allotment_model->get_room_details($allotted_room['room_id'])['room_type'];
			$entries = $this->edc_booking_model->get_guest_entries($app_num, $allotted_room['room_id']);
			...
			{	//add current room to list of available rooms
				...
				$data['rooms'][$sno++] = $this->edc_allotment_model->get_room_details($allotted_room['room_id']);
				$data['rooms_left']++;
			}
		}
	...
	function generate_bill($app_num, $name, $check_in) {
			...
			$data = $this->edc_booking_model->get_group_details($app_num, $name, $check_in); //returns app_num, name, designation, gender, address, email, contact, room_alloted, check_in and check_out, id_path
	...
	//	 $booking_details['purpose']="personal";
		...
		$total_sum = $this->bill_total_sum($app_num, $name, $check_in);
		$this->edc_booking_model->set_paid_data($app_num, $name, $check_in, $total_sum);
		//get basic details to show in receipt
		$data = $this->edc_booking_model->get_group_details($app_num, $name, $check_in); //returns app_num, name, designation, gender, address, email, contact, room_alloted, check_in and check_out, id_path
		//get rooms for that group
		$rooms = $this->edc_booking_model->get_guest_rooms($app_num, $name, $check_in);
	...
	function add_checkout($app_num, $room_alloted, $guest_name) {
		...
		$this->edc_booking_model->set_check_out($app_num);
	...
	{
		$this->auth_is('pce_da5');
		$config['upload_path'] = 'assets/files/edc_booking/'.$this->edc_booking_model->get_request_user_id($app_num).'/';
	...
	function display_booking_history(){
		...
		$occupance_history['booking_history'] = $this->edc_booking_model->get_room_occupance_history($data);

		foreach($occupance_history['booking_history'] as $key => $data)
		{
			$occupance_history['booking_history'][$key]['room_details'] = $this->edc_allotment_model->get_room_details($data['room_alloted']);
			...
			foreach($rooms as $room)
			{
				$room_details[$sno] =  $this->edc_allotment_model->get_room_details($room['room_alloted']);
				if($room_details[$sno]['room_type'] == 'Double Bedded AC')
					$room_details[$sno]['single'] = $this->edc_booking_model->check_single_room($data['app_num'], $data['name'], $data['check_in'], $room['room_alloted']);	//0 -> single in total (400/600), 1 -> single in group(200/300), 2 -> double in group (400/600)
				$sno++;
			}
			$data['rooms'] = $room_details;
			foreach($this->edc_booking_model->get_booking_details($data['app_num']) as $row){
...
// show the guest detail
	function show_guest_details($user_id, $app_num, $name, $check_in)
	{
		...
		$res = $this->edc_booking_model->get_guest_info($app_num, $name, $check_in);
		$res->user_id = $user_id;
		$this->drawHeader("Executive Development Center");
		$this->load->view('edc_booking/guest_info', $res);
		$this->drawFooter();
	}
}
